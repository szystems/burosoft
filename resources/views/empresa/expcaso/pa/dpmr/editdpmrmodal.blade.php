<!-- Modal -->
<div class="modal fade" id="editDpmrPaModal-{{ $dpmr->id }}" tabindex="-1" aria-labelledby="editDpmrPaModal-{{ $dpmr->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editDpmrPaModal-{{ $dpmr->id }}">
                    <i class="bi bi-pencil text-warning"></i> Editar Diligencia Para Mejor Resolver (PA)
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ url('update-dpmr-pa/'.$dpmr->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="audiencia_pa_id" value="{{ $dpmr->audiencia_pa_id }}">
                    ...
                    <div class="row gx-3">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea class="form-control @error('observaciones') is-invalid @enderror" 
                                      id="observaciones" name="observaciones" rows="3">{{ $dpmr->observaciones }}</textarea>
                            @error('observaciones')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('numero_folios') is-invalid @enderror" id="numero_folios_{{ $dpmr->id }}" name="numero_folios" value="{{ old('numero_folios', $dpmr->numero_folios) }}" min="1">
                            @error('numero_folios')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
