<!-- Modal -->
<div class="modal fade" id="editEvPaModal-{{ $ev->id }}" tabindex="-1" aria-labelledby="editEvPaModal-{{ $ev->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editEvPaModal-{{ $ev->id }}">
                    <i class="bi bi-pencil text-warning"></i> Editar Evacuación de Audiencia (PA)
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ url('update-ev-pa/'.$ev->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="row gx-3">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="datetime-local" name="fecha_hora_presentacion" class="form-control" value="{{ date('Y-m-d\TH:i', strtotime($ev->fecha_hora_presentacion)) }}" required>
                        ...
                        <input type="hidden" name="audiencia_pa_id" value="{{ $ev->audiencia_pa_id }}">
                    </div>
                </div>
