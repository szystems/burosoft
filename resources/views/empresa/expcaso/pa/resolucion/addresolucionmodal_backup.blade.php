                ...
                <div class="modal-body">
                    <div class="row gx-3">
                        ...
                        <div class="col-md-6 mb-3" id="tipoResolucionOtroDivPa" style="display: none;">
                            ...
                            <!-- Botones de prueba MÁS VISIBLES -->
                            <div class="mt-2">
                                <button type="button" onclick="testMostrar()" class="btn btn-success btn-sm">� MOSTRAR CAMPO</button>
                                <button type="button" onclick="testOcultar()" class="btn btn-danger btn-sm">� OCULTAR CAMPO</button>
                            </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="plazo_revocatoria" id="plazo_revocatoria_pa_add" class="form-control" onchange="togglePlazoRevocatoriaOtroPa(this.value)">
...
<script>
// Hacer las funciones globales para poder llamarlas desde onclick
...
// Función para plazo también global
window.togglePlazoRevocatoriaOtroPa = function(value) {
    console.log('=== TOGGLE PLAZO REVOCATORIA ===');
    ...
    try {
        ...
        if (!otroDiv || !otroInput) {
            console.error('❌ No se pudieron encontrar los elementos de plazo revocatoria');
            return;
        }
...
function initializeModalPA() {
    console.log('=== INICIALIZANDO MODAL PA ===');
    ...
    try {
        ...
        console.log('Select tipo resolución encontrado:', !!tipoResolucionSelect);
        console.log('Select plazo revocatoria encontrado:', !!plazoRevocatoriaSelect);
        
        // Manejar valores iniciales
        if (tipoResolucionSelect && tipoResolucionSelect.value === 'otro') {
            console.log('Valor inicial es otro, mostrando campo');
            toggleTipoResolucionOtroPa('otro');
        }
        ...
        if (plazoRevocatoriaSelect) {
            plazoRevocatoriaSelect.addEventListener('change', function() {
                console.log('📢 Change event plazo detectado, valor:', this.value);
                try {
                    togglePlazoRevocatoriaOtroPa(this.value);
                } catch (error) {
                    console.error('❌ ERROR al llamar togglePlazoRevocatoriaOtroPa:', error);
                }
            });
        }
        ...
        console.error('❌ ERROR en initializeModalPA:', error);
    }
}

// Múltiples formas de inicializar
document.addEventListener('DOMContentLoaded', initializeModalPA);

// También cuando se muestre el modal
$(document).on('shown.bs.modal', '#addResolucionPaModal', function () {
    console.log('📢 Modal PA mostrado, inicializando...');
    setTimeout(initializeModalPA, 100);
});

// Fallback con setTimeout
setTimeout(function() {
    if (document.getElementById('tipo_resolucion_pa_add')) {
        console.log('📢 Fallback: inicializando después de timeout');
        initializeModalPA();
    }
}, 500);
</script>
