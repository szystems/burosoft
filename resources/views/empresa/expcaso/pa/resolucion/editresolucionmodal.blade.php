<!-- Modal -->
<div class="modal fade" id="editResolucionPaModal-{{ $resolucion->id }}" tabindex="-1" aria-labelledby="editResolucionPaModal-{{ $resolucion->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editResolucionPaModal-{{ $resolucion->id }}">
                    <i class="bi bi-pencil text-warning"></i> Editar Resolución (PA)
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ url('update-resolucion-pa/'.$resolucion->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
