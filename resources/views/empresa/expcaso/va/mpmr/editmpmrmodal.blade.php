<!-- Modal -->
<div class="modal fade" id="editMpmrVaModal-{{ $mpmr->id }}" tabindex="-1" aria-labelledby="editMpmrVaModal-{{ $mpmr->id }}" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editMpmrVaModal-{{ $mpmr->id }}">
            ...
            <form action="{{ url('update-mpmr/'.$mpmr->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row gx-3">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="numero_folios_{{ $mpmr->id }}" class="form-label">Número de Folios</label>
                            <input type="number" class="form-control @error('numero_folios') is-invalid @enderror" 
                                   id="numero_folios_{{ $mpmr->id }}" name="numero_folios" 
                                   value="{{ old('numero_folios', $mpmr->numero_folios) }}" min="1">
                            @error('numero_folios')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
